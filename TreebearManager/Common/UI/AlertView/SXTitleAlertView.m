...
+ (instancetype)alertWithTitle:(NSString *)title confirmStr:(NSString *)confirmStr cancelStr:(NSString *)cancelStr{
    SXTitleAlertView *alert = [[self alloc] initWithFrame:UIApplication.sharedApplication.delegate.window.bounds];
    alert.title = title;
...
- (void)layoutSubviews{
    ...
    [self.cancleButton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self.bgImageView.mas_bottom).mas_offset(-10);
        make.size.mas_equalTo(CGSizeMake(contentW/2 - 15, 40));
    }];
